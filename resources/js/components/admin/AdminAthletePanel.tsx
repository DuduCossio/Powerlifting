...
export function AdminAthletePanel({ label, name, detail, weightLabel, accent, compact = false }: AdminAthletePanelProps) {
    ...
    return (
        <article className={`rounded border border-outline-variant bg-surface-container p-6 ${accent === 'primary' ? 'glow-active' : ''}`}>
            ...
            <div className="mt-6 flex justify-center gap-4 md:gap-8">
                {[0, 1, 2].map((index) => (
                    <div key={index} className="flex h-20 w-20 items-center justify-center rounded-full border-4 border-outline-variant bg-surface-container-highest transition-all md:h-24 md:w-24">
                        <Icon icon="material-symbols:circle" className="text-3xl text-on-surface-variant/20 md:text-4xl" />
                    </div>
                ))}
            </div>
